                                    ...
                                    std::string const &deviceIP)
{
    ...
    std::clog << "GvcpClient device is " << manufacturerName << " " << modelName << std::endl;
#endif
    return true;
}
                                   ...
                                   std::string const &deviceIP)
{
    ...
    if (regs.empty()) {
        std::clog << "GvcpClient can't read CCP (exclusive access ?)" << std::endl;
        releaseDevice();
        return false;
    }
...
void GvcpClient::readreg(uint8_t *pBuffer, uint64_t address, uint64_t length)
{
    ...
    std::memcpy(pBuffer, res.data(), static_cast<std::size_t>(length));
}
